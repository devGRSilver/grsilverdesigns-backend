        ...
        <form class="validate_form" action="{{ route('categories.update', encrypt($category->id)) }}" method="POST"
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-15">
                    ...
                    <select name="parent_id" class="form-control select2" id="parentCategorySelect">
                        <option value="">-- Main Category (This Is Main Category) --</option>
                        @foreach ($categories ?? [] as $cat)
                            <option value="{{ $cat->id }}"
                                {{ $category->parent_id == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <select name="metal_id" class="form-control" required>
                        ...
                        @foreach ($metals ?? [] as $metal)
                            <option value="{{ $metal->id }}"
                                {{ $category->metal_id == $metal->id ? 'selected' : '' }}>{{ $metal->name }}</option>
                        @endforeach
                    </select>
                    <small class="form-text text-muted">Choose the metal this category belongs to.</small>
                    @error('metal_id')
