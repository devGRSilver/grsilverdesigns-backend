$(document).ready(function () {
    ...
    $attachStorageBtn.off('click').on('click', async function () {
        const targetVariantIndex = $storageModal.data('target-variant-index');
        ...
        try {
            if (targetVariantIndex !== undefined) {
                await handleVariantImageAttachment(targetVariantIndex);
            } else {
    ...
    async function loadStorageImages() {
        ...
        try {
            ...
            if (targetVariantIndex !== undefined) {
                const $inputField = $(`#variant-images-input-${targetVariantIndex}`);
